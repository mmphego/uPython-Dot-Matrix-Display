...
class InitialSetUp:
    def __init__(self, config_dict, utc_shift=2):
        ...
        self.ssids = config_dict["wifi_config"]["ssid"]
        self.passwords = config_dict["wifi_config"]["password"]
    ...
    def wifi_config(self, disableAP=False):
        ...
        try:
            for ssid, password in zip(self.ssids, self.passwords):
                print("## Connecting to WiFi %s" % ssid)
                wlan = self.setup_wifi.wifi_connect(ssid, password)
                if wlan:
